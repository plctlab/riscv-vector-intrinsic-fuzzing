...
static std::string getRawPointerString(std::ostream &os, ValueBase *value,
  ...
  if (isOneDValue(value))
    os << value->dataTypeID << " *" << holder << " = " << value->id << ";\n";
...
static std::string getFlippedMaskReg(std::ostream &os,
  ...
  os << "vbool" << booleanSew << "_t " << flippedMaskReg
     << " = __riscv_vmandn_mm_b" os << "vbool" << booleanSew << "_t "
     << flippedMaskReg << " = __riscv_vmandn_mm_b" << booleanSew << "("
     << vecMaskSet << ", " << maskReg << ", vl);\n";
...
static void setVLMax(std::ostream &os, const TypeInfo &typeInfo) {
  os << " // generated by library/Operator.cpp setVLMax\n\t"
     << "int vlmax = " << typeInfo.setvlmaxTypeName << "();\n";
}
...
void storeVectorToOneD(std::ostream &os, const std::string rawBase,
  ...
  os << " // generated by library/Operator storeVectorToOneD\n\t";
  if (value->type == OneDBool) { // store bool into int8_t
    ...
    auto typeInfo = *value->typeInfo;
    os << "__riscv_vse" << typeInfo.sew.to_string() << "_v_" os << "__riscv_vse"
       << typeInfo.sew.to_string() << "_v_" << typeInfo.shortVectorTypeName
       << "(" << rawBase << ", " << vec << ", vl);\n\t";
  }
}
...
static void storeScalarToScalar(std::ostream &os, const std::string &lhs,
  ...
  os << "\t // generated by library/Operator.cpp storeScalarToScalar\n\t";
  os << "*" << lhs << " = " << rhs << ";\n\t";
}
...
struct CodeGenForVmsbfVmsifVmsof : CodeGenForOperator {
  ...
  std::string getLoopEndStoreMask(std::vector<std::string> args) {
    ...
    if (hasMA(op)) {
      ...
      os << "__riscv_vmandn_mm_b" << booleanSew << "(" << vecMO << ", "
         << vecM os << "__riscv_vmandn_mm_b" << booleanSew << "(" << vecMO
         << ", " << vecM << ", vl);\n";
    }
    return vecStored;
  }
  ...
  void getLoopEnd(const std::string &counter) {
    os << "}\n";
    os << "for (size_t vl;" << counter << " > 0; " << counter
       << " -= vl) {\n\t";
    {
      getVL(counter);
      if (!hasMask(op)) {
  ...
  void updateFirst(const std::string &first, std::vector<std::string> &args) {
    ...
    if (hasMask(op)) {
      ...
      os << first << " = __riscv_vfirst_m_b" << booleanSew << "_m(" << vecM
         << ", " os << first << " = __riscv_vfirst_m_b" << booleanSew << "_m("
         << vecM << ", " << vecVs2 << ", vl);\n";
    } else {
      os << first << " = __riscv_vfirst_m_b" << booleanSew << "(" << vecVs2 os
         << first << " = __riscv_vfirst_m_b" << booleanSew << "(" << vecVs2
         << ", vl);\n";
    }
  }
  void generateSingleOperatorCode() {
...
struct CodeGenForViota : CodeGenForOperator {
  ...
  void storeResult(const std::string &opResult, std::string &accumulateMask,
                   std::vector<std::string> &args) {
    os << opResult << " = __riscv_vadd_vx_" << vlTypeInfo.shortVectorTypeName os
       << opResult << " = __riscv_vadd_vx_" << vlTypeInfo.shortVectorTypeName
       << getOpSuffix(op) + "(";
    if (hasNonmask(op)) {
      if (hasTU(op))
        os << args[0] << ", ";
  ...
  void updateAccumulateMask(const std::string &accumulateMask,
    ...
    if (hasNonmask(op))
      os << accumulateMask << " += __riscv_vcpop_m_b"
         << getBooleanSew(vlTypeInfo) os << accumulateMask
         << " += __riscv_vcpop_m_b" << getBooleanSew(vlTypeInfo) << "("
      ...
      os << accumulateMask << " += __riscv_vcpop_m_b"
         << getBooleanSew(vlTypeInfo) os << accumulateMask
         << " += __riscv_vcpop_m_b" << getBooleanSew(vlTypeInfo) << "_m("
         << vecM << ", " << vecInputA << ", "
  ...
  std::vector<std::string> getIndexArguments() {
    ...
    }
    {
      if (hasMask(op)) {
        ...
        os << vecIdx << " = __riscv_vremu_vx_" os << vecIdx
           << " = __riscv_vremu_vx_"
           << inputIdx.second->typeInfo->shortVectorTypeName;
        os << "_m(" << vecM << ", " << vecIdx << ", " << opOutput.second->length
           << ", vl);\n";
      } else {
        os << vecIdx << " = __riscv_vremu_vx_" os << vecIdx
           << " = __riscv_vremu_vx_"
    ...
    os << vecIdx << " = __riscv_vmul_vx_" os << vecIdx << " = __riscv_vmul_vx_"
