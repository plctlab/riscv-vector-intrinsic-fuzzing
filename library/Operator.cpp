...
struct CodeGenForReductionOperator : CodeGenForOperator {
  ...
  void generateSingleOperatorCode() {
    ...
    CodeGenForOperator::getLoopStart(os, counter);
    {
      ...
      if (loaded.size() == 1) {
        if (hasTA(op)) {
          // TODO: find which parameter to delete.the "" is written by me to find which condition the vredxor belongs to
        ...
        if (hasTA(op) || hasTU(op)) { // tu or tam
          args = {"loaded.size() == 2|hasTA(op)|loaded[0]", loaded[1], vecReduction};
        } else { // normal _m
          args = {"loaded.size() == 2|loaded[0]", vecReduction, loaded[1], vecReduction};
        }
      } else if (loaded.size() == 3) { // tum
        args = {"loaded.size() == 3|loaded[0]", loaded[1], loaded[2], vecReduction};
      }
